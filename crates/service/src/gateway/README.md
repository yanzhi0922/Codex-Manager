...
- 进入 `ordered / balanced` 之前，还会先结合额度余量、当前并发和冷却状态做一轮轻量智能排序
...
- 聚合 API 池会先按手工 `sort` 做基础顺序，再在池内执行 round-robin 起点轮转
- 每个聚合 API 会记录运行期健康分、并发占用和最近测试结果；队首会优先命中更健康、负载更低的供应商
- 如果平台密钥显式绑定了某个聚合 API，该绑定仍然优先，不会被智能负载覆盖
- 跨池优先级仍保持“账号优先 / 聚合优先”；只有当前优先池整体耗尽或全部不可用时，才会回退到另一池
