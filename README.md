...
<p align="center">
  ...
  <a href="https://github.com/yanzhi0922/Codex-Manager">GitHub 仓库</a>|
  <a href="#赞助支持">赞助支持</a>
</p>
...
## 最近变更
- 当前最新版本：`v0.1.18`（2026-05-21）
- 项目已合并为 `Codex-Copilot`：会话扫描、诊断、Provider 迁移、导出与 `session_index.jsonl` 修复能力已进入主应用，不再需要单独启动 `codex-session-migrator`。
- 新增“能力驾驶舱”：参考 cockpit-tools 的产品分层方式，把账号池、网关、会话、平台 Key、多实例工作区和跨工具平台矩阵放到同一张产品地图里；仅借鉴产品结构，不复制第三方源码。
...
- 跨池优先级继续保持 `账号优先 / 聚合优先`；只有当前优先池整体不可用或耗尽后，才会回退到另一池，避免提前跨池打乱策略。
- 桌面端更新仓库、设置页 Release 跳转、Docker release compose 默认镜像仓库已全部对齐到 `yanzhi0922/Codex-Manager` / `ghcr.io/yanzhi0922`，避免 fork 发版后仍误指向上游仓库。
- 聚合 API 页面新增独立仪表盘：仅统计聚合 API 转发请求的累计请求数、成功/异常、令牌消耗、缓存命中与今日费用，不会混入官方账号池直连数据。
- 新增“聚合 API”管理页：可将多个第三方中转服务作为最小转发上游统一管理，支持按 `Codex / Claude` 分类、配置供应商名称 / 顺序 / URL / 密钥，并提供连通性测试。
- 平台密钥轮转现在支持 `账号轮转优先 / 聚合 API 轮转优先` 两种策略；首选路线耗尽后会自动回退到另一条路线，避免账号池和聚合 API 彼此孤立。
- `v0.1.15` 进一步把池内调度从“静态顺序”升级为“智能负载均衡”，同时保留你当前依赖的跨池优先语义。
...
- 发布链路继续统一收口：版本已提升到 `0.1.18`，workspace、前端包、Tauri 桌面端、版本一致性校验脚本和 README 版本说明已同步对齐。完整历史请看 [CHANGELOG.md](CHANGELOG.md)。
...
## 功能概览
- 能力驾驶舱：集中展示账号、网关、会话、平台 Key、多实例和跨工具平台矩阵的落地状态与下一步方向
...
## 页面展示
### 桌面端
- 能力驾驶舱：对照账号 / 实例 / 平台 / 运维四个视角，明确当前能力、下一阶段和跨工具调研边界
