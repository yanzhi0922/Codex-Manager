...
<p align="center">One local workspace for Codex account pools, gateway forwarding, and desktop session migration.</p>
...
Codex-Copilot combines Codex-Manager and Codex Session Migrator into one desktop/service product for accounts, usage, platform keys, a local gateway, and Codex Desktop sessions.
...
## Recent Changes
- Current latest version: `v0.1.18` (2026-05-21)
- The project is now unified as `Codex-Copilot`: session scanning, diagnostics, provider migration, export, and `session_index.jsonl` repair are built into the main app, so the standalone `codex-session-migrator` service is no longer required.
- Added the Capability Cockpit: inspired by cockpit-tools' product layering, it puts the account pool, gateway, sessions, platform keys, future multi-instance workspaces, and cross-tool platform matrix on one product map without copying third-party source code.
- The GitHub Actions release path now upgrades to `pnpm/action-setup@v5` and `docker/login-action@v4`, so it runs directly on the official Node 24 runtime without relying on a Node 20 compatibility override.
- GitHub Release notes now prefer the matching `CHANGELOG.md` section for the current version, so published releases ship with proper outward-facing notes instead of only autogenerated compare links.
...
- The release path stays unified: the product version is now `0.1.18`, and the workspace, frontend package, Tauri desktop app, release-version checks, and README version notes are all kept in sync. See [CHANGELOG.md](CHANGELOG.md) for the full history.
...
## Features
- Capability Cockpit: one product map for accounts, gateway, sessions, platform keys, multi-instance workspaces, and cross-tool platform readiness
...
- Session management: scan `~/.codex/sessions`, search by provider/workspace, inspect prompt previews, run diagnostics, migrate providers, export Markdown / HTML / JSON / JSONL / CSV / TXT, and rebuild `session_index.jsonl`
...
## Page Overview
### Desktop
- Cockpit: review the account, instance, platform, and operations perspectives with current status, next steps, and cross-tool boundaries
- Accounts: bulk import/export, refresh accounts and usage, plus low-quota / banned filters and reset-time display
- Sessions: built-in core capabilities from the former `codex-session-migrator`, including migration, export, diagnostics, and index repair
...
Thanks to everyone supporting Codex-Copilot. Your sponsorship and donations help keep the project actively maintained and steadily improved.
...
- WeChat: ProsperGao
- Telegram group: [Codex-Copilot TG Group](https://t.me/+OdpFa9GvjxhjMDhl)
- Community Group:
