...
## 剩余主要问题
- `crates/service/src/lib.rs` 仍有少量顶层入口模块尚未做同类子域门面收口，例如 `rpc_dispatch` 仍直接挂在根层，后续可评估是否按 transport / handlers / registry 继续下沉。
- `gateway/observability/http_bridge.rs` 与 `gateway/protocol_adapter.rs` 仍承担较多根入口转发职责，后续可继续做更清晰的 facade 分层。
- 测试目录虽然已去掉一批 wildcard 导入，但 `service/tests/*` 仍可以继续做测试支持层归并，减少分散 helper。

## 结论
当前 `service` 治理已从“半整理且阻塞编译”推进到“关键子域已门面化、根模块噪音明显下降、启动/协议适配/默认地址/RPC 回归持续全绿”的状态，已经具备继续做剩余根模块治理的稳定基线。
