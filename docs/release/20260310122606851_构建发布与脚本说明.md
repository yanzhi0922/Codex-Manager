...
- 本地桌面打包已与 CI 对齐，统一调用 `pnpm --dir apps dlx @tauri-apps/cli@2.10.1 build`
- 不再依赖本机额外安装 `cargo tauri`
...
- 发版前会先执行 `validate_release_version`，校验 workspace / 前端包 / Tauri 版本一致性，以及 `tag` 与 `ref` 是否指向同一 commit
- Release 正文会优先从 `CHANGELOG.md` 中提取当前版本段落；若未找到对应段落，才会回退到 GitHub 自动生成 notes
- 前端 `dist` 先单独构建一次，再被各平台打包 job 复用
- 输入：
  - `tag`：必填
  - `ref`：可选；留空时默认直接构建该 `tag`
...
# 强制发布为 pre-release
pwsh -NoLogo -NoProfile -File scripts/rebuild.ps1 `
  -AllPlatforms -GitRef main -ReleaseTag v0.1.17-beta.1 -GithubToken <token> -Prerelease true
```
...
```powershell
pwsh -NoLogo -NoProfile -File scripts/bump-version.ps1 -Version 0.1.17
```
